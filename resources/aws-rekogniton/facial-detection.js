...
module.exports.search_face = function(obj, callback){
	...
	rekognition.searchFacesByImage({
	 	...
	 	"FaceMatchThreshold": 60, //set minumum match in image send
	 	"Image": {
			'Bytes': new Buffer(obj.photo, 'base64')
	 	},
	 	"MaxFaces": 100 // set the number face detect in image send
	}, function(err, data) {
	 	if (err) {
			console.log(err)
			callback(err);
			...
			if(data.FaceMatches && data.FaceMatches.length > 0 && data.FaceMatches[0].Face)
			{
				...
				callback (data.FaceMatches.map((face)=>face.Face));
			} else {
